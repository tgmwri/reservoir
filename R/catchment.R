...
as.catchment <- function(id, down_id, data, area, res_data, branches, main_branch, res_wateruse = NULL, res_properties = NULL) {
    ...
    if (!is.null(res_properties)) {
        for (res in 1:length(reservoirs)) {
            id_without_catch = gsub(paste0(id, "_"), "", attr(reservoirs[[res]], "id"), fixed = TRUE)
            for (property in names(res_properties)) {
                if (!is.null(res_properties[[property]][[id_without_catch]])) {
                    reservoirs[[res]] = set_property(reservoirs[[res]], property, res_properties[[property]][[id_without_catch]])
                }
            }
        }
    }
...
calc_catchment_system.catchment_system <- function(system, yields = NULL, initial_storages = NULL, output_vars = c("storage", "yield")) {
    ...
    for (catch_name in catch_names) {
        catch_outlet_name = paste0(catch_name, "_outlet")
        if ((is.null(yields) || (!is.null(yields) && is.na(yields[catch_outlet_name]))) && is.null(attr(system[[catch_outlet_name]], "yield"))) {
            if (is.null(yields)) {
                yields = c()
            }
            yields[catch_outlet_name] = 0
        }
    }
